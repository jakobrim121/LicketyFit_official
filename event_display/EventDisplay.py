...
class EventDisplay:
    ...
    def plotEventDisplay(self,data, vmax, vmin, log_scale, color_map=plt.cm.plasma, color_norm=colors.LogNorm(), color_label=None,fig_width=None, show_zero=False, style=None):
        ...
        #print(coordinates[:, 0].shape)
            ...
            pmts = ax.scatter(coordinates[:, 0], coordinates[:, 1], c=data, s=10*scale*scale, cmap=color_map,vmax = vmax,vmin = vmin)
            
        fig.colorbar(pmts, ax=ax, pad=0, label=color_label)
        
        labels = range(0,106)
        self.label_mPMTs(labels)
