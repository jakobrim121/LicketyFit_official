...
def build_observables_from_event(ev, wcte, pe_scale=145.0):
    ...
    The time array uses NaN for PMTs with no hit. This is numerically equivalent
    to the original None-based masking in the likelihood, but much faster.
    """
    obs_pes = []
    obs_ts = []
...
def fit_track_with_minuit(
    ...
    This keeps the original fit logic by default:
      - strategy = 2
      - staged minimization (vertex -> direction -> full fit)
      - simplex + migrad at each stage
      - hesse at the end

    The speed-up is kept in the numeric plumbing around the fit:
      - pure float arrays for observables
      - no object-dtype time arrays
      - no repeated array conversions inside the FCN
      - lightweight emitter reuse inside the likelihood closure
    """
    if align_times:
